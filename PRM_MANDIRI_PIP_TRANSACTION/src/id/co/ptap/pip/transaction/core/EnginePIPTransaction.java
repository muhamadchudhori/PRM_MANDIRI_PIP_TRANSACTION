...
public class EnginePIPTransaction implements ParsingRecords {
	public Logger log = Logger.getLogger(EnginePIPTransaction.class.getName());
	private static final String ENTER = "\r\n";
	private static final String EXT_PROPERTY_URL = "db.prop.url";
	...
	public EnginePIPTransaction() {
		prop = SystemProperties.getInstance();
		try {
			prop.loadProperties(System.getProperty("transaction_debitedc.prop"));
		} catch (IOException e) {
			...
			if (tempFile.isFile()) {
				...
				if (prefixFile.startsWith(PREFIX_PROCESS_FILE)) {
					...
					this.log.info("FINISH PROCESS FILE : {" + this.fileName
							...
							+ " ms, TOTAL RECORDS : " + this.count + " rec.");
				}
			}
	}
	...
	private void proccessFile(String path, String fileProcess) {
		...
		try {
			...
			while ((line = this.in.readLine()) != null) {
				...
				buffer.append(parsingRecords(line, RECORD_LENGTH));
